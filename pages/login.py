...
def verify_credentials(username, password):
    conn = sqlite3.connect("database.db")
    cursor = conn.cursor()
    cursor.execute("SELECT username, password, email, photo_path FROM users WHERE username=?", (username,))
...
if use_face_login:
    ...
    if camera_photo and st.button("Login with Face"):
        ...
        conn = sqlite3.connect("database.db")
        cursor = conn.cursor()
        cursor.execute("SELECT username, password, email, photo_path FROM users")
